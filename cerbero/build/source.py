...
class Source (object):
    ...
    async def fetch(self, **kwargs):
        ...
        await self.fetch_impl(**kwargs)
        self._fetch_env_restore()

    async def fetch_impl(self):
        '''
        Fetch the sources
        '''
...
class Tarball(BaseTarball, Source):
    ...
    async def fetch_impl(self, redownload=False):
        ...
        if not redownload and os.path.isfile(cached_file) and self.verify(cached_file, fatal=False):
            ...
            return
        await super().fetch_impl(redownload=redownload)
...
class GitCache (Source):
    ...
    async def fetch_impl(self, checkout=True):
...
class Git (GitCache):
    ...
    def _already_fetched(self, common_source_recipes):
        for recipe in common_source_recipes:
            print(f'checking if step done for recipe {recipe.name}')
