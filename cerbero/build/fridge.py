...
class BinaryRemote (object):
    def fetch_binary(self, packages_names, local_dir, remote_dir):
        raise NotImplementedError

    def upload_binary(self, packages_names, local_dir, remote_dir, env_file):
        raise NotImplementedError
...
class FtpBinaryRemote (BinaryRemote):
    ...
    def fetch_binary(self, packages_names, local_dir, remote_dir):
        for filename in packages_names.values():
            if filename:
    ...
    def upload_binary(self, packages_names, local_dir, remote_dir, env_file):
        ...
        for filename in packages_names.values():
            if filename:
                local_filename = os.path.join(local_dir, filename)
                local_sha256_file = local_filename + '.sha256'
                sha256 = shell.file_sha256(local_filename)
                # .sha256 file contains both the sha256 hash and the filename, separated by a whitespace
                with open(local_sha256_file, 'w') as f:
                    f.write('%s %s' % (sha256.hex(), filename))
                upload_curl(local_filename,
                            os.path.join(self.remote, remote_dir, filename),
                            user=self.username,
                            password=self.password)
                upload_curl(local_sha256_file,
                            os.path.join(self.remote, remote_dir, filename + '.sha256'),
                            user=self.username,
                            password=self.password)
...
class Fridge (object):
    ...
    def fetch_binary(self, recipe):
        self.binaries_remote.fetch_binary(self._get_packages_names(recipe),
    ...
    def upload_binary(self, recipe):
        self.binaries_remote.upload_binary(self._get_packages_names(recipe),
