...
class FilesProvider(object):
    ...
    def _validate_library(self, file):
        '''
        Validate libraries in the prefix. Unfortunately the filename might vary
        ...
        # like this to avoid changing all the library validator implementations
        pattern = r"\*?(?P<libname>[^\*]*)\*?{}\*?".format(libext)
        m = re.match(pattern, f)
        f = m.group('libname')
        libsmatch = self._validatelib(self.config, f[3:], self.config.prefix,
                                     libdir, libext, libregex)
        return libsmatch

    def _validate_library_pdb(self, file):
        f = os.path.basename(file)
    ...
    def _validate_existing(self, files, only_existing=True):
        ...
        vfs = []
        for f,validator in files.items():
            if not validator:
                validator = self._validate_file
            validated = validator(f)
            # Warn about missing files
            if not validated:
                m.warning("Missing on-disk files for {} with validator {}".format(f, validator.__name__))
            else:
    ...
    def devel_files_list(self, only_existing=True):
        ...
        devfiles.update(self._search_girfiles())
        devfiles.update(self._search_devel_libraries())
    ...
    def files_list_by_category(self, category, only_existing=True):
        ...
        '''
        return self.files_list_by_categories([category], only_existing=only_existing)
    ...
    def _search_libraries(self, files):
        ...
        for f in files:
            ...
            libs[pattern] = self._validate_library
        return libs
        ...
        for category in self.categories:
            ...
            for x in self._get_category_files_list(category):
                ...
                if self.have_pdbs():
                     ...
                     devel_libs[pdb] = self._validate_library_pdb
